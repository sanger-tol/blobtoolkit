...
workflow BUSCO_DIAMOND {
    ...
    dir_a = dir_a.map { meta,table -> [id, table.copyTo("$table.parent/archaea_odb10_full_table.tsv")] }.collect()
    dir_b = dir_b.map { meta,table -> [id, table.copyTo("$table.parent/bacteria_odb10_full_table.tsv")] }.collect()
    dir_e = dir_e.map { meta,table -> [id, table.copyTo("$table.parent/eukaryota_odb10_full_table.tsv")] }.collect()
