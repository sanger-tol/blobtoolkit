...
def merge_all(freq, mononuc, mosdepth, countbusco):
    ...
    combo_fw = freq_fw.merge(mononuc_fw).rename(columns={"ID": "sequence", "GC_prop": "gc", "Prop_Ns": "n", "N": "ncount"})

    count_df = pd.read_csv(countbusco, sep="\t").rename(columns={"ID": "sequence"})
    for f in mosdepth:
        tag = os.path.basename(f).replace(".regions.bed.gz", "")
        cov_df = pd.read_csv(f, compression="gzip", sep="\t", names=['sequence', 'start', 'end', tag + '_cov'])
