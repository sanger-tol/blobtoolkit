...
- `read_mapping/`
  - `<datatype>/`
    - `<specimen>/<run>/` (or `<sample>/` if this information is provided in the samplesheet)
      - `<assembly>.<datatype>.<sample>.coverage.<window_size>.bed.gz`: BedGraph file with per-window coverage.
      - `<assembly>.<datatype>.(<specimen>.<run>|<sample>).minimap2.coverage.<window_size>.bed.gz`: same as above when alignments are generated by this pipeline using minimap2 (`--align`).
...
- `base_content/`
  - `k1/`, `k2/`, `k3/`, `k4/`
    - `<assembly>.<mono|di|tri|tetranuc>.<window_size>.tsv.gz`: tab-separated files with counts of every _k_-mer for k &le; 4. The first three columns correspond to the coordinates (sequence name, start, end), followed by each _k_-mer.
